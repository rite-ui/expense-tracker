          ...
          return (
            <button
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all border w-full text-left
                ${isActive
                  ? isDark ? 'bg-brand/10 text-white border-brand/20' : 'bg-brand/10 text-brand border-brand/20'
                  : isDark ? 'text-white/50 border-transparent hover:bg-white/5 hover:text-white' : 'text-gray-500 border-transparent hover:bg-gray-50 hover:text-gray-800'
                }`}
            >
              <span className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm shrink-0 transition-all
                ${isActive ? 'bg-brand text-white shadow-lg shadow-brand/30' : isDark ? 'bg-white/5' : 'bg-gray-100'}`}>
                {n.icon}
              </span>
      ...
      <div className={`p-4 border-t ${isDark ? 'border-white/5' : 'border-gray-100'}`}>
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium border transition-all mb-3
            ${isDark ? 'border-white/5 bg-white/5 text-white/50 hover:text-white' : 'border-gray-100 bg-gray-50 text-gray-500 hover:text-gray-800'}`}
        >
          <span>{isDark ? '☀️' : '🌙'}</span>
          <span className="truncate">{isDark ? 'Light Mode' : 'Dark Mode'}</span>
        </button>

        {/* ✅ FIXED: User Badge Section */}
        <div className={`flex items-center gap-2.5 px-2 py-2 rounded-xl border
          ...
          <div className="w-8 h-8 rounded-lg bg-brand flex items-center justify-center text-white text-xs font-bold shrink-0">
            {/* Backend se aane wale username ke liye getInitial */}
            {getInitial(auth?.name || 'U')}
          </div>

          <div className="flex-1 min-w-0">
            <div className={`text-[11px] font-bold truncate ${isDark ? 'text-white' : 'text-gray-800'}`}>
              {/* ✅ Auth object se name display hoga */}
              {auth?.name || 'User'}
            </div>
            <div className={`text-[9px] font-mono opacity-50 ${isDark ? 'text-white' : 'text-gray-400'}`}>
              member
            </div>
          </div>
