...
export default function AnalyticsPage() {
  ...
  const kpis = [
    { label: 'Savings Rate',      value: `${savingsRate}%`,    sub: 'of income saved',           color: 'text-income'  },
    { label: 'Avg / Transaction', value: expenses.length ? formatCurrency(Math.round((totalIncome + totalExpense) / expenses.length)) : '₹0', sub: `${expenses.length} records`, color: 'text-brand-light' },
    { label: 'Top Category',      value: breakdown[0]?._id || '—', sub: breakdown[0] ? formatCurrency(breakdown[0].total) : 'no data', color: 'text-warning' },
    { label: 'Active Months',     value: new Set(summary.map(s => `${s._id.month}-${s._id.year}`)).size, sub: 'months tracked', color: 'text-info' },
  ]
  ...
  return (
    <div className="flex flex-col gap-4 animate-fadeIn">
      ...
      <div className="grid grid-cols-4 gap-4">
        {kpis.map((k, i) => (
          <div key={i} className={`${cardBase} p-5 animate-slideUp animate-stagger-${i + 1}`}>
            <div className={`text-[10px] font-bold tracking-[1.2px] uppercase font-mono mb-2 ${dark ? 'text-white/30' : 'text-gray-400'}`}>
              {k.label}
            </div>
            <div className={`text-2xl font-extrabold tracking-tight ${k.color}`}>{k.value}</div>
            <div className={`text-[11px] font-mono mt-1.5 ${dark ? 'text-white/25' : 'text-gray-400'}`}>{k.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className={`${cardBase} p-5`}>
          <div className="mb-4">
            <div className={`text-sm font-bold ${dark ? 'text-white' : 'text-gray-800'}`}>Income vs Expenses</div>
            <div className={`text-[11px] font-mono mt-0.5 ${dark ? 'text-white/30' : 'text-gray-400'}`}>monthly comparison</div>
          </div>
        ...
        <div className={`${cardBase} p-5`}>
          <div className="mb-4">
            <div className={`text-sm font-bold ${dark ? 'text-white' : 'text-gray-800'}`}>Savings Trend</div>
            <div className={`text-[11px] font-mono mt-0.5 ${dark ? 'text-white/30' : 'text-gray-400'}`}>monthly net savings</div>
          </div>
          <ResponsiveContainer width="100%" height={220}>
      ...
      <div className="grid grid-cols-[1fr_1.4fr] gap-4">
        <div className={`${cardBase} p-5`}>
          <div className="mb-4">
            <div className={`text-sm font-bold ${dark ? 'text-white' : 'text-gray-800'}`}>Category Distribution</div>
            <div className={`text-[11px] font-mono mt-0.5 ${dark ? 'text-white/30' : 'text-gray-400'}`}>all expenses</div>
          </div>
          {pieData.length > 0 ? (
        ...
        <div className={`${cardBase} p-5`}>
          <div className="mb-5">
            <div className={`text-sm font-bold ${dark ? 'text-white' : 'text-gray-800'}`}>Category Breakdown</div>
            <div className={`text-[11px] font-mono mt-0.5 ${dark ? 'text-white/30' : 'text-gray-400'}`}>ranked by spending</div>
          </div>
          <div className="flex flex-col gap-4">
            {breakdown.length === 0 && (
              <div className={`text-center py-6 text-xs font-mono ${dark ? 'text-white/25' : 'text-gray-400'}`}>
                ...
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1.5">
                      <span className={`text-xs font-semibold ${dark ? 'text-white/80' : 'text-gray-700'}`}>{b._id}</span>
                      <span className={`text-[11px] font-mono ${dark ? 'text-white/40' : 'text-gray-400'}`}>{pct}%</span>
                    </div>
                  ...
                  <div className="text-right shrink-0">
                    <div className={`text-xs font-bold font-mono ${dark ? 'text-white/80' : 'text-gray-700'}`}>
