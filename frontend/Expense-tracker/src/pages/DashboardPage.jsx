...
export default function DashboardPage({ 
  ...
  // ✅ 1. ACCURATE CALCULATIONS (Frontend Side)
  // Hum INCOME_CATEGORIES ka use karke ensure karenge ki data sahi categorize ho
  const incomeTransactions = expenses.filter(e => 
    e.type === 'income' || INCOME_CATEGORIES.includes(e.category)
  );
  const expenseTransactions = expenses.filter(e => 
    e.type === 'expense' && !INCOME_CATEGORIES.includes(e.category)
  );

  const calculatedIncome = incomeTransactions.reduce((acc, curr) => acc + curr.amount, 0);
  const calculatedExpense = expenseTransactions.reduce((acc, curr) => acc + curr.amount, 0);
  ...
  // ✅ 2. BAR CHART LOGIC (Income vs Expense Fix)
  const barData = (() => {
    const map = {}
    summary.forEach(s => {
      ...
      // Agar category Income list mein hai, toh hamesha income bar mein dalo
  ...
  // ✅ 3. PIE CHART LOGIC (Sirf Expenses dikhane ke liye)
  const pieData = breakdown
    .filter(b => !INCOME_CATEGORIES.includes(b._id)) // Income categories ko Spending chart se hataya
    .map(b => ({
      name: b._id, 
      value: b.total, 
  ...
  return (
    <div className="flex flex-col gap-5 animate-fadeIn w-full overflow-x-hidden pb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard 
          label="Net Balance" value={Math.abs(calculatedBalance)} type="balance" icon="💎"
          sub={calculatedBalance >= 0 ? '▲ Positive' : '▼ Negative'} delay="animate-stagger-1" 
        />
        <StatCard 
          label="Total Income" value={calculatedIncome} type="income" icon="📈"
          sub={`${incomeTransactions.length} sources`} delay="animate-stagger-2" 
        />
        <div className="sm:col-span-2 lg:col-span-1">
          <StatCard 
            label="Total Expenses" value={calculatedExpense} type="expense" icon="📉"
            sub={`${expenseTransactions.length} items`} delay="animate-stagger-3" 
          />
        </div>
      </div>
        ...
        <div className={`${cardBase} p-4 md:p-5 animate-slideUp`}>
          ...
          {pieData.length > 0 ? (
            <div className="flex flex-col sm:flex-row lg:flex-col items-center lg:items-stretch gap-6">
              ...
              <div className="flex flex-col gap-2.5 w-full sm:w-1/2 lg:w-full max-h-40 overflow-y-auto pr-1 custom-scrollbar">
                {pieData.map((d, i) => (
                  <div key={i} className="flex items-center justify-between text-[11px]">
                    ...
                    <span className={`font-mono font-bold shrink-0 ml-2 ${isDark ? 'text-white/70' : 'text-gray-700'}`}>
